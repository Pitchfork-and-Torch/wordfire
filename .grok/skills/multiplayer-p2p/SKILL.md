...
ping), and `candidateType` (`host`/`srflx` = direct, `relay` = TURN).
`useP2PRoom` fetches `/api/ice` once on join for short-lived Cloudflare TURN
credentials. Server env: `TURN_KEY_ID` + `TURN_KEY_API_TOKEN` (never `VITE_`).
Missing env: STUN-only. Optional `.env`: `VITE_STUN_URLS` (comma-separated).
