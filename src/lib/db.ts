...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    // HTTP driver works on Cloudflare Workers/Pages, Node, and edge runtimes.
    // Do NOT use node-postgres (`pg`) here — TCP sockets fail on CF isolates.
    // Deploy-time migrations still use `pg` in scripts/migrate.mjs (Node only).
    const { neon, types } = await import("@neondatabase/serverless");
    // Match PGLite/pg result parity for bigint / date / interval.
    try {
      types.setTypeParser(OID_INT8, Number);
      types.setTypeParser(OID_DATE, identity);
      types.setTypeParser(OID_INTERVAL, identity);
    } catch {
      // types API is best-effort on the HTTP driver; rows are still JSON-safe.
    }
    const sqlHttp = neon(databaseUrl!, { fullResults: false });
