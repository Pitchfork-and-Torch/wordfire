...
import { iceCandidateDisposition } from "./turn-filter";
...
export interface PeerInfo {
  ...
  rttMs: number | null;
  /** Watchdog gave up after MAX_RECOVERY_ATTEMPTS. */
  terminal?: boolean;
}
...
export class P2PRoom {
  ...
  private bindHangUp(): void {
    if (this.hangUpBound || typeof window === "undefined") return;
    window.addEventListener("pagehide", this.onHangUp);
    window.addEventListener("beforeunload", this.onHangUp);
    this.hangUpBound = true;
  }

  private unbindHangUp(): void {
    if (!this.hangUpBound || typeof window === "undefined") return;
    window.removeEventListener("pagehide", this.onHangUp);
    window.removeEventListener("beforeunload", this.onHangUp);
    this.hangUpBound = false;
  }

  private postLeave(): void {
    ...
    try {
      if (typeof navigator !== "undefined" && typeof navigator.sendBeacon === "function") {
        ...
        if (navigator.sendBeacon("/api/rtc", blob)) return;
      }
    } catch {
  ...
  close(): void {
    ...
    // Tell the circle before tearing the mesh so seats drop without waiting
    // on signaling TTL. Best-effort: the channel may already be dead.
    try {
      this.send({ t: "bye", playerId: this.opts.selfId });
    } catch {
      // Hang-up still continues via the leave POST below.
    }
  ...
  private async onSignal(
    ...
    try {
      if (kind === "offer" || kind === "answer") {
        ...
        if (slot.ignoreOffer) {
          // Drop ICE queued for the colliding offer; it must not flush onto ours.
        ...
        const disposition = iceCandidateDisposition(
          slot.ignoreOffer,
          Boolean(slot.pc.remoteDescription),
        );
  ...
  private async readCandidateType(slot: PeerSlot): Promise<void> {
    // relay = TURN (minted via /api/ice); srflx/host = direct path.
    try {
