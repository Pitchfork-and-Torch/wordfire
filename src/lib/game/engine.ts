...
/** he'll / they're / I'm - not obfuscation like shi't or fu'ck. */
const CONTRACTION_RHS = /^(ll|ve|re|m)$/i;

export function isEnglishContraction(token: string): boolean {
  const m = token.match(/^[\p{L}\p{N}]+['']([\p{L}\p{N}]+)[.!?...,]?$/u);
  return Boolean(m && CONTRACTION_RHS.test(m[1]!));
}

export function formatLiveTally(words: StoryWord[]): string {
  const stats = computeStoryStats(words);
  const parts = stats.wordCount === 1 ? "1 part" : `${stats.wordCount} parts`;
  const sentences =
    stats.sentenceCount === 1 ? "1 sentence" : `${stats.sentenceCount} sentences`;
...
export function sanitizeContribution(
  ...
  for (const t of tokens) {
    ...
    if (banned.has(core) && !isEnglishContraction(token)) {
