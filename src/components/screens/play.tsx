...
import { formatLiveTally, formatStoryText } from "@/lib/game/engine";
      ...
      <header className="relative z-10 mb-3 flex shrink-0 items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <Badge variant="secondary" className="tabular-nums">
            {formatLiveTally(words)}
          </Badge>
          <Button
            variant="ghost"
            size="icon"
            aria-label="Copy the story so far"
            disabled={words.length === 0}
            onClick={() => {
              const text = formatStoryText(words);
              void navigator.clipboard.writeText(text).then(
                () => toast.success("Copied the fire so far"),
