...
| `kick` | Host removes peer from circle (soft); receivers apply only when data-channel `from` matches `by` |
...
| Peer leaves mid-turn | `bye` + signaling leave; roster drop; token moves to next seated; host fails over if they left |
